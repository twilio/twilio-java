...
public class SipIpAddressFetcher extends Fetcher<SipIpAddress> {
    ...
    public SipIpAddress execute(final TwilioRestClient client) {
        Request request = new Request(HttpMethod.GET, "/SIP/IpAccessControlLists/" + IpAccessControlListSid + "/IpAddresses/" + sid + ".json");
        Response response = client.request(request);

        if (response.getStatusCode() != TwilioRestClient.HTTP_STATUS_CODE_OK) {
            throw new ApiException("Unable to retrieve SIP IpAddress for Sid " + sid + ": [" + response.getStatusCode() + "] " + response.getContent());
        }
