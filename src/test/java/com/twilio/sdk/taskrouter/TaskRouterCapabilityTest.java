...
public class TaskRouterCapabilityTest {
    ...
    public void testDeprecatedWorker() throws Exception {
        ...
        assertEquals("https://taskrouter.twilio.com/v1/Workspaces/WS456/Workers/WK789/Channels/**", p.get("url"));
        assertEquals("GET", p.get("method"));
    ...
    public void testGenerateDefaultWorkerToken() throws Exception {
        final TaskRouterWorkerCapability capability =
                new TaskRouterWorkerCapability("AC123", "foobar", "WS456", "WK789");
        final String token = capability.generateToken();
        final String[] parts = token.split("\\.");
        assertEquals(3, parts.length);
        final String payload = parts[1];
    ...
    public void testGenerateTaskQueueToken() throws Exception {
        final TaskRouterTaskQueueCapability capability =
                new TaskRouterTaskQueueCapability("AC123", "foobar", "WS456", "WQ111");
        final String token = capability.generateToken();
        final String[] parts = token.split("\\.");
        assertEquals(3, parts.length);
        final String payload = parts[1];
