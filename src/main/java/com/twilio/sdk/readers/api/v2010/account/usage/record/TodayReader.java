...
import com.twilio.sdk.resources.api.v2010.account.usage.record.Today;
...
public class TodayReader extends Reader<Today> {
    ...
    public ResourceSet<Today> execute(final TwilioRestClient client) {
        ...
        Page<Today> page = pageForRequest(client, request);
     ...
     */
    @Override
    public Page<Today> nextPage(final String nextPageUri, final TwilioRestClient client) {
        Request request = new Request(
            HttpMethod.GET,
            nextPageUri,
            client.getAccountSid()
     ...
     */
    protected Page<Today> pageForRequest(final TwilioRestClient client, final Request request) {
        ...
        Page<Today> result = new Page<>();
        result.deserialize("usage_records", response.getContent(), Today.class, client.getObjectMapper());
