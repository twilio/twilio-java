...
public class StoreFetcher extends Fetcher<Store.FetchStoreResponse> {

    private String pathStoreId;

    public StoreFetcher(final String pathStoreId) {
    ...
    private Response makeRequest(final TwilioRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "Store fetch failed: Unable to connect to server"
            );
    ...
    @Override
    public Store.FetchStoreResponse fetch(final TwilioRestClient client) {
        Response response = makeRequest(client);
        return Store.FetchStoreResponse.fromJson(
            response.getStream(),
            client.getObjectMapper()
        );
    }
    ...
    @Override
    public TwilioResponse<Store.FetchStoreResponse> fetchWithResponse(
        final TwilioRestClient client
    ) {
        Response response = makeRequest(client);
        Store.FetchStoreResponse content = Store.FetchStoreResponse.fromJson(
            response.getStream(),
            client.getObjectMapper()
        );
