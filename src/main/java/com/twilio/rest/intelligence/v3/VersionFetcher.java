...
public class VersionFetcher extends Fetcher<Version.FetchVersionResponse> {

    private String pathId;
    private Integer pathVersion;

    public VersionFetcher(final String pathId, final Integer pathVersion) {
        this.pathId = pathId;
    ...
    private Response makeRequest(final TwilioRestClient client) {
        ...
        path = path.replace("{" + "version" + "}", this.pathVersion.toString());
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "Version fetch failed: Unable to connect to server"
            );
    ...
    @Override
    public Version.FetchVersionResponse fetch(final TwilioRestClient client) {
        Response response = makeRequest(client);
        return Version.FetchVersionResponse.fromJson(
            response.getStream(),
            client.getObjectMapper()
        );
    }
    ...
    @Override
    public TwilioResponse<Version.FetchVersionResponse> fetchWithResponse(
        final TwilioRestClient client
    ) {
        Response response = makeRequest(client);
        Version.FetchVersionResponse content =
            Version.FetchVersionResponse.fromJson(
                response.getStream(),
                client.getObjectMapper()
            );
