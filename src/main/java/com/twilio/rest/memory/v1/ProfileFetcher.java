...
public class ProfileFetcher extends Fetcher<Profile.FetchProfileResponse> {
    ...
    public ProfileFetcher setTraitGroups(final String traitGroups) {
    ...
    private Response makeRequest(final TwilioRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "Profile fetch failed: Unable to connect to server"
            );
    ...
    @Override
    public Profile.FetchProfileResponse fetch(final TwilioRestClient client) {
        Response response = makeRequest(client);
        return Profile.FetchProfileResponse.fromJson(
            response.getStream(),
            client.getObjectMapper()
        );
    }
    ...
    @Override
    public TwilioResponse<Profile.FetchProfileResponse> fetchWithResponse(
        final TwilioRestClient client
    ) {
        Response response = makeRequest(client);
        Profile.FetchProfileResponse content =
            Profile.FetchProfileResponse.fromJson(
                response.getStream(),
                client.getObjectMapper()
            );
