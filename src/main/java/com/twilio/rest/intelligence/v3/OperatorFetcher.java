...
public class OperatorFetcher extends Fetcher<Operator.FetchOperatorResponse> {

    private String pathId;

    public OperatorFetcher(final String pathId) {
        this.pathId = pathId;
    }
    ...
    private Response makeRequest(final TwilioRestClient client) {
        ...
        if (response == null) {
            throw new ApiConnectionException(
                "Operator fetch failed: Unable to connect to server"
            );
    ...
    @Override
    public Operator.FetchOperatorResponse fetch(final TwilioRestClient client) {
        Response response = makeRequest(client);
        return Operator.FetchOperatorResponse.fromJson(
            response.getStream(),
            client.getObjectMapper()
        );
    }
    ...
    @Override
    public TwilioResponse<Operator.FetchOperatorResponse> fetchWithResponse(
        final TwilioRestClient client
    ) {
        Response response = makeRequest(client);
        Operator.FetchOperatorResponse content =
            Operator.FetchOperatorResponse.fromJson(
                response.getStream(),
                client.getObjectMapper()
            );
