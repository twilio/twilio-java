# Contributing to `twilio-java`

We'd love for you to contribute to our source code and to make `twilio-java`
...
of other developers, as we all have the same goal: make `twilio-java` as good as
it can be.
...
* **Major Changes** that you wish to contribute to the project should be
  discussed first with `twilio-java` contributors in an issue or pull request so
...
* **`twilio-java` Version(s)** - is it a regression?
...
* Run the full `twilio-java` test suite (aliased by `make test`), and ensure
  that all tests pass.
...
* Re-run the `twilio-java` test suite to ensure tests are still passing.
